...
angular.module('TOX2ADMINAPP').controller('MatchLeagueCtrl', [
  '$rootScope', '$scope', '$filter',
  'MatchLeagueService', 'PApi', 'settings', 'init',
  function(
    $rootScope, $scope, $filter,
    MatchLeagueService, PApi, settings, init,
  ) {
    ...
    $scope.Add = function() {
      PApi.StartLoading();
      MatchLeagueService.Add($scope._item, function(data, defer) {
        if (data.failure) defer.reject(data.failure);
        defer.resolve();
      }).then(function() {
        return MatchLeagueService.List($scope._query, function(data, defer) {
    ...
    $scope.Modify = function() {
      PApi.StartLoading();
      MatchLeagueService.Modify($scope._item, function(data, defer) {
          if (data.failure) return defer.reject(data.failure);
          defer.resolve();
        }
      ).then(function(legacy) {
        return MatchLeagueService.List(
          $scope._query,
          function(data, defer) {
            if (data.failure) return defer.reject(data.failure);
            $scope.RenderList(data.lastPage, data.docs);
    ...
    $scope.Remove = function(doc) {
      PApi.StartLoading();
      MatchLeagueService.Remove(doc._id, function(data, defer) {
          if (data.failure) return defer.reject(data.failure);
          defer.resolve();
        }
      ).then(function(legacy) {
    ...
    $scope.BulkRemove = function() {
      ...
      PApi.StartLoading();
      var queue = MatchLeagueService.Remove(list[0]);
      for (var i = 1; i < list.length; i++) {
        (function(ii) {
          queue = queue.then(function() {
            return MatchLeagueService.Remove(list[ii]);
          });
        })(i);
      }
    ...
    $scope.List = function() {
      PApi.StartLoading();
      MatchLeagueService.List($scope._query, function(data) {
