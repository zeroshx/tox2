...
angular.module('TOX2ADMINAPP').controller('WithdrawalCtrl', [
  '$rootScope', '$scope',
  'WithdrawalService', 'PApi', 'settings', 'init',
  function(
    $rootScope, $scope,
    WithdrawalService, PApi, settings, init
  ) {
    ...
    $scope.BulkAccept = function() {
      ...
      PApi.StartLoading();
      var queue = WithdrawalService.Accept(list[0]);
      for(var i = 1; i < list.length; i++) {
        (function(ii) {
          queue = queue.then(function () {
            return WithdrawalService.Accept(list[ii]);
          });
        })(i);
      }
    ...
    $scope.BulkCancel = function() {
      ...
      if(list.length === 0) return;
      PApi.StartLoading();
      var queue = WithdrawalService.Cancel(list[0]);
      for(var i = 1; i < list.length; i++) {
        (function(ii) {
          queue = queue.then(function () {
            return WithdrawalService.Cancel(list[ii]);
          });
        })(i);
      }
      queue.then(function (legacy) {
