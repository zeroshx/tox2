angular.module('TOX2ADMINAPP')
  .controller('DistributorCtrl', function($rootScope, $scope, $routeParams, $window, $anchorScroll, CRUDService, PublicService) {
      ...
      if (mode === 'UPDATE' || mode === 'VERIFY') {
        ...
        for (var i in $scope.docs) {
          if ($scope.docs[i]._id === id) {
            ...
            $scope.targetManagerUid = $scope.docs[i].manager.uid;
            $scope.targetManagerNick = $scope.docs[i].manager.nick;
            $scope.targetSite = $scope.docs[i].site;
            $scope.targetStatusPoint = $scope.docs[i].statusPoint;
            $scope.targetBonusWin = $scope.docs[i].bonus.win;
            $scope.targetBonusLose = $scope.docs[i].bonus.lose;
            $scope.targetJoinStyle = $scope.docs[i].joinStyle;
          }
        }
        if (!docCheck) {
    ...
    ****************************************************************************/
    $scope.Create = function() {
      CRUDService.Create($scope.baseUrl).run({
        ...
        managerUid: $scope.targetManagerUid,
        managerNick: $scope.targetManagerNick,
        statusPoint: $scope.targetStatusPoint,
    ...
    ****************************************************************************/
    ...
    $scope.ResetTarget = function() {
      ...
      $scope.targetManagerUid = null;
      $scope.targetManagerNick = null;
      $scope.targetSite = null;
      $scope.targetStatusPoint = null;
