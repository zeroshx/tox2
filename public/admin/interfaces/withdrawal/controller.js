angular.module('TOX2ADMINAPP')
  .controller('WithdrawalCtrl', function($rootScope, $scope, $routeParams, $filter, $window, $anchorScroll, CRUDService, PublicService) {
    ...
    ****************************************************************************/
    ...
    $scope.Accept = function(id) {
      CRUDService.Update($scope.baseUrl + '/accept', id).run({}, function(res) {
        if (res.failure) {
          $scope.validator.type = 'error';
          $scope.validator.message = res.failure;
          $anchorScroll(0);
    ...
    $scope.Cancel = function(id) {
      CRUDService.Update($scope.baseUrl + '/cancel', id).run({}, function(res) {
        if (res.failure) {
          $scope.validator.type = 'error';
          $scope.validator.message = res.failure;
          $anchorScroll(0);
