angular.module('TOX2ADMINAPP')
  .controller('UserCtrl', function($rootScope, $scope, $routeParams, $filter, $window, $anchorScroll, CRUDService, PublicService) {
      ...
      if (mode === 'UPDATE' || mode === 'VERIFY') {
        ...
        for (var i in $scope.docs) {
          if ($scope.docs[i]._id === id) {
            ...
            $scope.targetMoney = $scope.docs[i].chip;
            ...
            $scope.targetDistributor = $scope.docs[i].distributor.name;
    ...
    ****************************************************************************/
    ...
    $scope.RenderList = function() {
      for (var m in $scope.docs) {
        $scope.docs[m].stat.profit = $scope.docs[m].stat.deposit - $scope.docs[m].stat.withdrawal;
      }
    };
